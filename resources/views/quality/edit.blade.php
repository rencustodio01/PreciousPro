...
<div class="page-header">
    <div><h2>Edit Inspection</h2><small>Inspection #{{ $qualityInspection->id }}</small></div>
...
<div class="row justify-content-center">
    <div class="col-lg-7">
        <div class="form-card">
            ...
            @endif
            <form method="POST" action="{{ route('quality.update', $qualityInspection) }}">
                @csrf @method('PUT')
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <select name="production_id" class="form-select @error('production_id') is-invalid @enderror" required>
                            ...
                            @foreach($productions as $prod)
                                <option value="{{ $prod->id }}" {{ old('production_id', $qualityInspection->production_id) == $prod->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="inspection_date" class="form-control @error('inspection_date') is-invalid @enderror" value="{{ old('inspection_date', $qualityInspection->inspection_date->format('Y-m-d')) }}" required max="{{ now()->format('Y-m-d') }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="result" class="form-select @error('result') is-invalid @enderror" required>
                            <option value="Pass" {{ old('result', $qualityInspection->result) == 'Pass' ? 'selected' : '' }}>✅ Pass</option>
                            <option value="Fail" {{ old('result', $qualityInspection->result) == 'Fail' ? 'selected' : '' }}>❌ Fail</option>
                        </select>
                        @error('result')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-12">
                        <label class="form-label">Remarks</label>
                        <textarea name="remarks" class="form-control @error('remarks') is-invalid @enderror" rows="3" maxlength="150">{{ old('remarks', $qualityInspection->remarks) }}</textarea>
                        @error('remarks')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-12 d-flex justify-content-end gap-2 pt-2">
                        <a href="{{ route('quality.index') }}" class="btn btn-outline-secondary">Cancel</a>
                        <button type="submit" class="btn btn-gold"><i class="bi bi-check-lg me-1"></i> Update Inspection</button>
                    </div>
                </div>
            </form>
        </div>
