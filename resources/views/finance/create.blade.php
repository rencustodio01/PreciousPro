@extends('layouts.app')
@section('title', 'Add Finance Record')

@section('content')
<div class="page-header">
    <div><h2>Add Finance Record</h2><small>Record a production cost entry</small></div>
            ...
            <form method="POST" action="{{ route('finance.store') }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="cost_type" class="form-select @error('cost_type') is-invalid @enderror" required>
                            ...
                            @foreach(['Material','Labor','Overhead'] as $ct)
                                <option value="{{ $ct }}" {{ old('cost_type') == $ct ? 'selected' : '' }}>{{ $ct }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="record_date" class="form-control @error('record_date') is-invalid @enderror" value="{{ old('record_date', now()->format('Y-m-d')) }}" required max="{{ now()->format('Y-m-d') }}">
                    ...
                    <div class="col-12 d-flex justify-content-end gap-2 pt-2">
                        ...
                        <button type="submit" class="btn btn-gold"><i class="bi bi-check-lg me-1"></i> Save Record</button>
                    </div>
                </div>
            </form>
        </div>
